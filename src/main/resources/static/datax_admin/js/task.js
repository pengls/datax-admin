...
(function ($, window, document) {
    ...
    task.edit = function () {
        ...
        var tkContent = JSON.parse(rowData.content);
    ...
    task.del = function () {
        ...
        layer.confirm('是否删除选中的数据源？', {
            ...
            $.post(rootPath + "/task/del", {taskIds: checkResult.join(',')}, function (data) {
                if (data.flag) {
                    common.alertSuc('成功删除任务', function () {
                        dtable.reload(table);
                    });
                } else {
                    common.alertError(data.resMsg);
    ...
    function initTable() {
        table = dtable.init({
            ...
            "columnDefs": [
                ...
                {
                    "render": function (data, type, row) {
                        return '<span class="label label-warning" onclick="task.showTaskContent(\'' + row.id + '\')">点击查看</span>';
                    },
                    "targets": 5
                },
                {
    ...
    task.showTaskContent = function (taskId) {
        var rowData = dtable.getCheckRowData('taskTable', taskId);
        common.model('任务详情', rowData.content, 'default');
    }
    ...
    function loadTables(dsId, suc) {
        $.post(rootPath + "/dts/tables", {dsId: dsId}, function (res) {
            if (res.flag) {
                suc(res.data);
            } else {
    ...
    function loadColumns(dsId, tableName, suc) {
        $.post(rootPath + "/dts/columns", {dsId: dsId, tableName: tableName}, function (res) {
            if (res.flag) {
                suc(res.data);
            } else {
                common.alertError(res.resMsg);
